...
import { loadPlayerCoins, updatePlayerCoins, saveMatchResult } from '@/lib/playerData';
...
export default function GamePage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        ...
        setAuthenticatedUsername(session.user.user_metadata?.username || 'Player');
      }
    });
  }, [router]);
  ...
  useEffect(() => {
      ...
      supabase.auth.getSession().then(({ data: { session } }) => {
        ...
        socket.emit('joinGame', { 
          ...
          if (response.success) {
            ...
            // Game is full, try to find random game
            if (response.reason === 'game_full') {
              console.log('🚫 Game full, finding random game...');
              socket.emit('findRandomGame', (randomResponse: any) => {
                socket.disconnect();
                setTimeout(() => {
                  router.push(`/game/${randomResponse.gameId}`);
                }, 500);
              });
            }
            // Match ended, redirect to new game
            else if (response.reason === 'match_ended') {
              console.log('⏰ Match ended, redirecting to new game:', response.gameId);
              socket.disconnect();
              setTimeout(() => {
                router.push(`/game/${response.gameId}`);
              }, 1000);
            } else {
              setJoinError(`Failed to join game: ${response.reason || 'unknown error'}`);
    ...
    socket.on('matchEnded', async (data) => {
      ...
      if (data.matchData && supabase.auth.getSession) {
        const { data: sessionData } = await supabase.auth.getSession();
        if (sessionData?.session?.user) {
          await saveMatchResult(
            gameId,
            data.matchData.winnerId || null,
            data.matchData.winnerName || null,
            data.matchData.durationSeconds || 600,
            data.matchData.playerResults || []
          );
        }
      }
    ...
    socket.on('gameReady', (data) => {
      console.log('🎮 New game ready:', data.gameId);
  ...
  }, [gameId, router, authenticatedUsername]);
  ...
  const handleBuyBuff = (buffType: string) => {
    ...
    // Update local player coins immediately for UI responsiveness
    const config = BUFF_CONFIG[buffType];
    if (currentPlayer && config) {
      currentPlayer.coins -= config.price;
      
      // Save to database
      supabase.auth.getSession().then(({ data: { session } }) => {
        if (session?.user) {
          updatePlayerCoins(session.user.id, currentPlayer.coins);
        }
      });
    }
    ...
    return (
      <div className="w-screen h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-xl text-gray-400 mb-2">Joining game...</div>
          <div className="text-gray-500">Game ID: {gameId}</div>
          {joinError && <div className="text-red-400 mt-4">{joinError}</div>}
        </div>
      </div>
    );
  }
      ...
      <div className="border-b border-gray-700 bg-gray-950 px-8 py-4 flex justify-between items-center">
        ...
        <div className="flex gap-8 text-xs text-gray-400">
          <div>
            ...
            <div className="text-purple-400 font-mono text-xs">{gameId}</div>
          </div>
          <div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          >
            ...
            {shopMenuOpen && nearbyShop && (
              <div 
                className="absolute inset-0 flex items-center justify-center z-50 bg-black bg-opacity-70 pointer-events-auto"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="bg-gray-900 border-2 border-cyan-400 rounded-lg p-8 max-w-md w-full shadow-2xl pointer-events-auto">
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {leaderboard.length === 0 ? (
                ...
                leaderboard.map((entry, index) => {
                  const now = Date.now();
                  const activeBuff = entry.buffs?.find(b => b.expiresAt > now);
                  const buffDisplay = activeBuff 
                    ? ` [${activeBuff.type.toUpperCase()} ${Math.ceil((activeBuff.expiresAt - now) / 1000)}s]`
                    : '';
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <span className="text-yellow-400 font-bold w-6 flex-shrink-0">#{index + 1}</span>
                        <span className="truncate">{entry.name}{buffDisplay}</span>
                      </div>
                      <div className="text-right text-yellow-400 font-bold flex-shrink-0 ml-2">
                        {entry.kills}
                      </div>
                    </div>
                  );
