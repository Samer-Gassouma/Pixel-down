...
function generateShops(obstacles) {
    const shops = [];
    const NUM_SHOPS = 1;
...
function findValidSpawnLocation(obstacles) {
    let attempts = 0;
...
setInterval(() => {
    ...
    for (const [gameId, game] of games) {
        if (game.players.size === 0) {
            ...
            if (game.postMatchTimeout)
                clearTimeout(game.postMatchTimeout);
            games.delete(gameId);
        }
    }
    // Update each game
    for (const [gameId, game] of games) {
        ...
        if (elapsedTime > MATCH_DURATION && !game.matchEnded) {
            ...
            // Find winner
            const winner = Array.from(game.players.values()).reduce((a, b) => a.kills > b.kills ? a : b);
...
io.on("connection", (socket) => {
    ...
    socket.on("joinGame", (data, callback) => {
        ...
        socket.data.gameId = requestedGameId;
    ...
    socket.on("buyBuff", (buffType) => {
        ...
        if (!buff || player.coins < buff.price)
            return;
